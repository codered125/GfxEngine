...
int main()
{
	...
	GLuint fakeFloor = TextureLoading::LoadTexture("Images/Brick.jpg");


	glm::mat4 FOV = glm::perspective(ourCamera.GetZoom(), (GLfloat)SCREEN_WIDTH / (GLfloat)SCREEN_HEIGHT, 0.1f, 1000.0f);
	
	GLuint fbo, cubeVAO, cubeVBO, planeVAO, planeVBO, quadVAO, quadVBO;
	...
	glVertexAttribPointer(1, 2, GL_FLOAT, GL_FALSE, sizeof(GLfloat) * 8, (GLvoid *)(6 * sizeof(GLfloat)));
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(base.planeVertices), &base.planeVertices, GL_STATIC_DRAW);
	glEnableVertexAttribArray(0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//floor
		glBindVertexArray(planeVAO);
		glBindTexture(GL_TEXTURE_2D, fakeFloor);
